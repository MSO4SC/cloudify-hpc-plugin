...
** `memory`: Specify the real memory required per node.  Different units can be specified using the suffix [`K|M|G|T`]. Default `""`.
** `output`: Define the file where to gather the standard output of the job. Default `""`.
** `error`: Define the file where to gather the standard error output. Default `""`.
** `mail-user`: Email to receive notification of job state changes. Default `""`.
** `mail-type`: Type of event to be notified by mail, can define several events separated by comma. Valid values `NONE, BEGIN, END, FAIL, TIME_LIMIT, REQUEUE, ALL`. Default `""`.
** `reservation`: Allocate resources for the job from the named reservation. Default `""`.
** `qos`: Request a quality of service for the job. Default `""`.
